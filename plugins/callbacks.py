...
from plugins.settings.inputs import handle_settings_callback
...
async def button(bot, update):
    if await handle_settings_callback(bot, update):
        return
    if update.data == "home":
        ...
        if not thumbnail:
            await update.answer("⚠️ You didn't set any custom thumbnail yet!", show_alert=True)
        else:
            await update.answer()
            await bot.send_photo(update.message.chat.id, thumbnail, f"{E.CREATE} <b>Yᴏᴜʀ Cᴜsᴛᴏᴍ Tʜᴜᴍʙɴᴀɪʟ</b>",
                               reply_markup=types.InlineKeyboardMarkup([[
                                   types.InlineKeyboardButton("🗑 Dᴇʟᴇᴛᴇ Tʜᴜᴍʙɴᴀɪʟ",
                                                              callback_data="deleteThumbnail")
                               ]]))
    elif update.data == "deleteThumbnail":
        await db.set_thumbnail(update.from_user.id, None)
        await update.answer("🗑 Thumbnail deleted! Default thumbnail will be used now.", show_alert=True)
